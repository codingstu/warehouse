...
router.post('/arrivestore', (req, res) => {
  ...
  // 判断货物是否存在 存在则只添加数量
  const sql1 = `select * from arrivestore where goodsname = '${goodsname}'`
  // 1. 准备sql
  ...
  conn.query(sql1, (err, data) => {
    ...
    if (data.length > 0) {
      ...
      // 操作数据库
      const sql2 = `update arrivestore set  goodsquantity ="${goodsquantity}" where goodsname = '${goodsname}'`
      ...
      // 不存在 入库
      // 1. 准备sql
      const sql3 = `insert ignore into arrivestore(goodscoding,goodsname,goodscategory,imgUrl,goodsdesc,goodsquantity) values("${goodscoding}", "${goodsname}", "${goodscategory}", "${imgUrl}","${goodsdesc}","${goodsquantity}")`
      // 2. 执行sql
  ...
  conn.query(sql1, (err, data) => {
    ...
    if (data.length > 0) {
      ...
      if (parseInt(data[0].goodsquantity) > parseInt(leavequantity)) {
        // 出库
        goodsquantity =
          parseInt(data[0].goodsquantity) - parseInt(leavequantity)
        // console.log(goodsquantity )
        // 操作数据库
        const sql2 = `update arrivestore set  goodsquantity ="${goodsquantity}" where goodsname = '${goodsname}'`
        conn.query(sql2, (err, data) => {
        ...
        // 删除货物
        const sql3 = `delete from arrivestore where goodsname = '${goodsname}'`
        conn.query(sql3, (err, data) => {
          if (err) throw err
          if (data.affectedRows > 0) {
...
router.get('/goodslist', (req, res) => {
  ...
  let sql = `select * from arrivestore where 1=1`
  if (goodscoding) {
...
router.get('/goodsdel', (req, res) => {
  ...
  const sql = `delete from arrivestore where goodsname = '${goodsname}'`
  conn.query(sql, (err, data) => {
    if (err) throw err
    if (data.affectedRows > 0) {
...
router.post('/goodsedit', (req, res) => {
  ...
  const sql = `update arrivestore set goodscoding="${goodscoding}", goodsname="${goodsname}", goodscategory="${goodscategory}", 
	 imgUrl="${imgUrl}", goodsdesc="${goodsdesc}",goodsquantity="${goodsquantity}" where goodsname="${goodsname}"`
